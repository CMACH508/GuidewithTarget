...
system(paste0("rm ",data_add,cellline,"_Parsed_POI.txt"))
system(paste0("rm ", data_add,cellline,"_Parsed_Filter_POI.txt"))
...
ParseEachQuery <- function(query_num){
  ...
  write.table(file=paste0(data_add,cellline,"_Parsed_POI.txt"),df2,row.names=FALSE,col.names=FALSE,quote=FALSE,append=TRUE,sep="@")
  write.table(file=paste0(data_add,cellline,"_Parsed_Filter_POI.txt"),truedf2,row.names=FALSE,col.names=FALSE,quote=FALSE,append=TRUE,sep="@")
  print(query_num)
}

no_of_cores <- 2
cl <- makePSOCKcluster(no_of_cores)
clusterExport(cl, c("rownum_query","result","data_add","cellline"))
